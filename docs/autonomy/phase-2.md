...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------- | ----- |
| 2.1 | The answer-bank ladder **shall** be replaced by typed intents per `autonomy-plan.md` §R3 ("Replace the answer-bank ladder with typed intents"), such that a resolution carries `{concept, polarity, class, provenance}` and a polarity that cannot be established **defers**, never inverts. **Budget: declare before pickup** (C11).                                                                        | `implementer` | SHALL |
| 2.2 | Bank-sourced **free-text** answers over a length threshold **shall** defer on the unattended path. `SKIP_TYPES` (`answer-bank.mjs`) skips only `file` and `richtext`, so a textarea labelled "Anything else we should know?" is a fillable target today. This defers a **field**, not an application. **Budget: declare before pickup.**                                                                     | `implementer` | SHALL |
| 2.3 | A per-form **disclosure declaration**: the plan **shall** name the bank ids it will disclose, and an unusual set defers. The read side of the fact base is currently a lookup keyed on an attacker-chosen string (`answer-bank.mjs`'s `normalizeQuestion` over the page's own label, at the exact-bank lookup) and nothing counts how many distinct facts one form pulls. **Budget: declare before pickup.** | `implementer` | SHALL |
