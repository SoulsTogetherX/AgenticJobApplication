...
from `clockedPage`'s existing `cdp_calls` counter (`bench-apply.mjs`). Every column in the
...
**The CI gate, implementable as written** (`ci-engineer` wires it, `qa` owns the harness):
...
throughput under the label N=8 (verified when this was written: `tests/fixtures/boards/server.mjs`
defined a single greenhouse route). **Landed 2026-08-02:** `server.mjs` now serves
`/boards.greenhouse.io/fixture-emp-<n>/jobs/<id>` and takes `origins: N`, which binds N listeners on
N ports — distinct ports are distinct origins, so the precondition this gate depends on now holds.
`bench-runner` **shall** still assert that observed max-in-flight equals the requested concurrency
and fail the run if it does not; the fixture being _capable_ of 8 origins is not the run _using_
them.
