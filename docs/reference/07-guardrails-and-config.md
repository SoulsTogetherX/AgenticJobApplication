...
| `roles`         | `title_keywords` (17), `hard_filter` (43), `soft_filter` (68), plus three **optional, absent-safe** keys not yet present in the file: `title_rank` (retargets `recommend.mjs`'s title scoring), `exclude_body` (retargets the body-disqualifier vocabulary — a **term list, never a switch**; an empty list is treated as absent, not as "reject on nothing"), `search_query` (retargets the sweep's search text). Absent reproduces today's behaviour exactly; setting one **replaces** the built-in, the same convention as `remote_synonyms`. Documented in `scripts/leads/{recommend,find-jobs}.mjs` |
