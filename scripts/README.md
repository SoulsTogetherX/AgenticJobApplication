# `scripts/` — the five externally-pinned files

**Owner:** `ci-engineer`. **This directory is not where code lives.** The
pipeline moved to `src/` in the 2026-08-27 re-layout. Five files stayed behind,
...
it holds exactly six tracked entries — the five pinned files plus this README —
and, separately, that the **invocable** set (everything that is not a `.md`) is
exactly those five. Adding one more invocable file fails the build.
...
It was six until **2026-08-28**, when the user repointed the Windows Scheduled
Task to `src\auto\cycle.cmd` and `auto/cycle.cmd` was deleted the same hour.
That is the lifecycle every shim here is meant to have: it exists to keep one
external consumer working, and it goes the moment that consumer moves.
...
`tests/quality/shims.test.mjs` asserts parity for as long as the shims exist,
and it asserts it against a **denial**, not an allowed command. That distinction
was earned: the original test fed each hook a command it should permit, and an
adversarial pass showed a two-line shim that ran nothing still passed — a
do-nothing hook also exits 0 with empty stdout. The probes now require
`guard-bash` to deny a `git checkout main`, `guard-files` to deny a write
outside the project, and `prettify` to actually change a misformatted file's
bytes.
...
**The three hook shims** — after the user edits `.claude/settings.json` to
invoke `node src/hooks/<name>.mjs`, and the hooks are live-fire verified (edit a
scratch file and observe prettier run; attempt a `git checkout main` and observe
the refusal).

**`auto/cycle.cmd` — done, 2026-08-28.** The precedent is worth keeping, because
it shows what "verified" has to mean. The order was: repoint the task, confirm
`schtasks /Query /XML` reports the new `<Command>`, run the new target **from a
foreign working directory** (the task has no _Start In_, so only `%~dp0` makes
the repo root resolvable — a probe run from the repo would have passed even if
self-location were broken), and only then delete the shim. That probe is also
what exposed the bare-LF batch defect now pinned by `shims.test.mjs`.

Until a shim's consumer moves, deleting it breaks a guardrail or a scheduled run
silently, which is the failure mode this whole arrangement exists to avoid.
