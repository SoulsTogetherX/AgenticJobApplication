...
//
// Usage: node scripts/apply-profile.mjs [--proposal profile/profile.proposed.yaml]
//        [--target profile/profile.yaml] [--allow-edits] [--allow-removals]
import fs from 'node:fs';
import path from 'node:path';
import { loadYamlFile, buildFactIndex } from './lib.mjs';
...
if (!fs.existsSync(proposalPath)) {
  console.error(`No proposal found at ${proposalPath}. The update-profile skill writes it for user review first.`);
  process.exit(2);
}
...
try {
  ...
  console.error(`Current profile failed to parse (${e.message}) — fix it before applying updates.`);
  process.exit(1);
}
...
const changed = [...targetIdx.keys()].filter(
  (id) => proposalIdx.has(id) && proposalIdx.get(id).text !== targetIdx.get(id).text
);
const added = [...proposalIdx.keys()].filter((id) => !targetIdx.has(id));

const problems = [];
if (removed.length && !allowRemovals) {
  problems.push(`Facts would be DELETED (pass --allow-removals only with user approval): ${removed.join(', ')}`);
}
if (changed.length && !allowEdits) {
  problems.push(`Facts would be REWRITTEN (pass --allow-edits only with user approval): ${changed.join(', ')}`);
}
if (problems.length) {
