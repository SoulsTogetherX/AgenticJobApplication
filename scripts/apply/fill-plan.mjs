...
// The engine is static and shared across every job; only the plan changes per
// job. Both get embedded into the generated bootstrap — see buildDriverSource.
...
async (page) => {
  // Both strings below were read/built in an ordinary Node process (this
  ...
  // dynamic import. See fill-page.js's sandbox notes for the full story.
  const sources = [${JSON.stringify(engineSrc)}, ${JSON.stringify(planSrc)}]
  // page.evaluate + eval, never addScriptTag: CDP-level evaluation is not
  // subject to the page's script-src the way an injected <script> element
  // is, so this is the one path that works on a nonce-based CSP board
  // (Ashby) as well as a board with none (Greenhouse). Do not "fix" this
  // back to addScriptTag.
...
function main() {
  ...
  // Read here — an ordinary Node process — never inside the generated
  // driver, which runs in a vm sandbox with no fs. See buildDriverSource().
  const engineSrc = fs.readFileSync(ENGINE_PATH, "utf8")
  fs.writeFileSync(jsPath, buildDriverSource(plan, engineSrc))
