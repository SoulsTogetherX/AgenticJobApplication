...
//
// Usage:
//   node scripts/discover-boards.mjs --candidates <file.yaml|file.json>
//   node scripts/discover-boards.mjs --type greenhouse --slug acme --company "Acme"
    ...
    for (const r of accepted) {
      console.log(
        `  ${r.company} (${r.label})\n` +
          ...
          `    node scripts/manage-sources.mjs add --type ${r.type} --slug ${r.label.split(":")[1]} --company "${r.company}"\n`,
      )
    }
  } else {
