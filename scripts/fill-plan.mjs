...
// every field through scripts/answer-bank.mjs (profile + answer bank only), and
// writes:
...
//
// Usage: node scripts/fill-plan.mjs <slug> [--scan <path>] [--url <url>]
...
export function resolveFields(fields, { profile, answers, script } = {}) {
  const args = [
    script ?? path.join(ROOT, "scripts", "answer-bank.mjs"),
    "--fields",
...
function main() {
  ...
  if (!slug) {
    console.error("usage: node scripts/fill-plan.mjs <slug> [--scan <path>]")
    process.exit(2)
  }
  ...
  fs.writeFileSync(
    jsPath,
    "// Generated by scripts/fill-plan.mjs — do not edit by hand.\n" +
      "window.__ajPlan = " +
      JSON.stringify(plan) +
      "\n",
  )
