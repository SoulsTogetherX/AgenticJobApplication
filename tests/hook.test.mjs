...
function runHook(payload) {
  ...
  let decision = null;
  if (res.stdout.trim()) decision = JSON.parse(res.stdout).hookSpecificOutput?.permissionDecision ?? null;
  return { status: res.status, decision };
}

const edit = (file) => JSON.stringify({ tool_name: 'Edit', tool_input: { file_path: file } });
