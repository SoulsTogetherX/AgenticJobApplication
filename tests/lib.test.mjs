...
import {
  ...
  validateContext, validateJob,
} from '../scripts/lib.mjs';

const FIX = path.join(path.dirname(fileURLToPath(import.meta.url)), 'fixtures');

test('extractNumbers normalizes separators and suffixes', () => {
  const n = extractNumbers('Served 1,200 users, 99.9% uptime, 45+ stars, C++17, ≤250 ms, GPA 3.75');
  assert.deepEqual([...n].sort(), ['1200', '17', '250', '3.75', '45', '99.9'].sort());
});
...
test('extractMonthYears finds month-year tokens', () => {
  const d = extractMonthYears('Jan 2024 – Present, graduated Jun 2023, and June 2022');
  assert.deepEqual([...d].sort(), ['Jan 2024', 'Jun 2022', 'Jun 2023'].sort());
});

test('techTermsIn finds terms with tricky boundaries', () => {
  const found = techTermsIn('Used C++ and Node.js with React Native; GitHub is not Git term-wise... but Git alone is.');
...
test('buildFactIndex indexes every fixture fact id uniquely', () => {
  ...
  for (const id of ['summary-fs', 'exp-acme', 'exp-acme-b1', 'exp-acme-b2', 'prj-demo-b1', 'skill-lang', 'edu-state', 'a-001']) {
...
test('buildFactIndex throws on duplicate ids', () => {
  const profile = {
    ...
    experience: [{ id: 'dup', title: 't', company: 'c', dates: 'd', bullets: [] }],
  };
  assert.throws(() => buildFactIndex(profile, { answers: [] }), /Duplicate fact id/);
});
