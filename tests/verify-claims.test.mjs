...
function verify(mode, file, extra = []) {
  const res = spawnSync(process.execPath, [
    path.join(ROOT, 'scripts', 'verify-claims.mjs'),
    mode, path.join(FIX, file),
    '--profile', path.join(FIX, 'profile.yaml'),
    '--answers', path.join(FIX, 'answers.yaml'),
    ...extra,
  ], { cwd: ROOT, encoding: 'utf8' });
  let report = null;
  try { report = JSON.parse(res.stdout); } catch { /* usage errors print no JSON */ }
...
test('faithful cover letter passes with job context', () => {
  const { status, report } = verify('cover-letter', 'good-cover-letter.md', ['--job', path.join(FIX, 'job.json')]);
  assert.equal(status, 0, JSON.stringify(report?.violations));
});
...
test('invented number in a cited bullet fails (R3)', () => {
  ...
  assert.ok(report.violations.some((v) => v.rule === 'R3' && v.detail.includes('5000')));
});

test('tech terms absent from the profile fail (R6)', () => {
  const { status, report } = verify('resume', 'bad-unknown-tech.md');
  assert.equal(status, 1);
  const terms = report.violations.filter((v) => v.rule === 'R6').map((v) => v.detail);
...
test('bullet without a fact annotation fails (R1)', () => {
  ...
  assert.equal(status, 1);
  assert.ok(report.violations.some((v) => v.rule === 'R1' && v.detail.includes('Led a team')));
});

test('citing a nonexistent fact id fails (R2)', () => {
  const { status, report } = verify('resume', 'bad-unknown-fact-id.md');
  assert.equal(status, 1);
  assert.ok(report.violations.some((v) => v.rule === 'R2' && v.detail.includes('exp-nonexistent-b9')));
});
...
test('dishonest cover letter fails on numbers, dates, and tech (R4/R5/R6)', () => {
  const { status, report } = verify('cover-letter', 'bad-cover-letter.md', ['--job', path.join(FIX, 'job.json')]);
  assert.equal(status, 1);
  const rules = new Set(report.violations.map((v) => v.rule));
  assert.ok(rules.has('R4'), 'invented numbers (7 years, 2,000,000 rps) should fail R4');
  assert.ok(rules.has('R5'), 'invented date (Aug 2021) should fail R5');
  assert.ok(rules.has('R6'), 'invented tech (Rust, AWS) should fail R6');
  const r6 = report.violations.filter((v) => v.rule === 'R6').map((v) => v.detail).join(' ');
...
test('cover letter may reference the job title/company even when the posting body is off-limits', () => {
  ...
  const { status } = verify('cover-letter', 'good-cover-letter.md', ['--job', path.join(FIX, 'job.json')]);
  assert.equal(status, 0);
});

test('usage errors exit 2', () => {
  assert.equal(verify('resume', 'does-not-exist.md').status, 2);
  const res = spawnSync(process.execPath, [path.join(ROOT, 'scripts', 'verify-claims.mjs'), 'badmode', 'x.md'], { cwd: ROOT, encoding: 'utf8' });
